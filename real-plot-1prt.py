...
class SerializationSchema(SerializationSchema):
    def __init__(self, topic='m4-downsampled-topic'):
        ...
        self._j_serialization_schema = gateway.jvm.org.apache.flink.connector.kafka.sink.KafkaRecordSerializationSchema.builder() \
            ...
            .setKeySerializationSchema(j_string_serialization_schema) \
            .build()

    def serialize(self, element: list, context=None) -> bytes:
        try:
...
def main():
    ...
    subset_size = 5
    X_train_normalized = X_train_normalized[:subset_size]
    X_test_normalized = X_test_normalized[:subset_size]
    print(f"Using subset: X_train_normalized shape={X_train_normalized.shape}, X_test_normalized shape={X_test_normalized.shape}")
    ...
    for idx, series in enumerate(np.concatenate([X_train_normalized, X_test_normalized], axis=0)):
        ...
        producer.flush()
        print(f"Sent series {idx} to Kafka")
    producer.close()
    ...
    env.set_parallelism(1)  # Match partitions and CPU cores
    #env.get_config().set_execution_timeout_millis(30000)
    # Kafka Source
    ...
    def _get_model():
        if _model_instance[0] is None:
            ...
            _model_instance[0] = keras.models.load_model(
    ...
    def process_element(element):
        try:
            ...
            # Stop processing after total_series elements
            if processed_count[0] > total_series:
                print(f"Reached {total_series} elements, stopping processing")
                raise StopIteration(f"Processed {total_series} elements")
    ...
    print("\n--- Starting Plotly Dash Dashboard for Real-Time Visualization ---")
    
    # Kafka Consumers for input and output topics
    ...
    def consume_kafka():
        try:
            ...
            message_count = {'input': 0, 'output': 0, 'paired': 0}
            while True:
                input_messages = input_consumer.poll(timeout_ms=1000)
                if not input_messages:
                    print("No new messages from m4-input-topic")
                for topic_partition, messages in input_messages.items():
                    for message in messages:
                        key = message.key
                        input_buffer[key] = np.array(message.value, dtype=np.float32)
                        message_count['input'] += 1
                        print(f"Received input series {key}, length={len(input_buffer[key])}, total input messages={message_count['input']}")
                        if key in output_buffer:
                            recent_series.append((input_buffer[key], output_buffer[key]))
                            message_count['paired'] += 1
                            print(f"Paired series {key} for visualization, total paired={message_count['paired']}")
                            del input_buffer[key]
                            del output_buffer[key]
                output_messages = output_consumer.poll(timeout_ms=1000)
                if not output_messages:
                    print("No new messages from m4-downsampled-topic")
                for topic_partition, messages in output_messages.items():
                    for message in messages:
                        key = message.key
                        output_buffer[key] = np.array(message.value, dtype=np.float32)
                        message_count['output'] += 1
                        print(f"Received output series {key}, length={len(output_buffer[key])}, total output messages={message_count['output']}")
                        if key in input_buffer:
                            ...
                            del input_buffer[key]
                            del output_buffer[key]
                print(f"Current recent_series size: {len(recent_series)}, message counts: {message_count}")
                time.sleep(0.1)
        except Exception as e:
    ...
    # Buffer to pair series by key
    input_buffer = {}
    output_buffer = {}
    recent_series = deque(maxlen=5)  # Store up to 5 recent series
    ...
    # Dashboard layout
    app.layout = html.Div([
        html.H1("Real-Time Downsampling Visualization"),
    ...
    def update_plot(n):
        ...
        if not recent_series:
            ...
            return go.Figure()
        fig = go.Figure()
        for i, (original, downsampled) in enumerate(recent_series):
            print(f"Adding series {i+1}: original length={len(original)}, downsampled length={len(downsampled)}")
        ...
        fig.update_layout(
            title='Real-Time Original vs Downsampled Time Series',
            xaxis_title='Sample',
    ...
    if not dash_thread.is_alive():
        print("Dash thread failed to start")
    else:
        print("Dash thread started successfully")
    # --- Existing Training and Evaluation (Unchanged) ---
    ...
    # Plot results (static matplotlib visualization)
    ...
    # Keep the script running to allow Dash to continue
    print("Script completed. Dash dashboard will continue running.")
    try:
        while True:
            time.sleep(1)
